...
//          通过自定义注解的方式定义一个类似通过拦截器权限一样的注解类型
        ...
        if (StrUtil.isBlank(token)) {
            throw new ServiiceException(Constants.CODE_401, "无token，请重新登录");
        }
        // 获取 token 中的 user id
        String userId;
        try {
            ...
            throw new ServiiceException(Constants.CODE_401, "token验证失败，请重新登录");
        }
        // 根据token中的userid查询数据库
        User user = userService.getById(userId);
        if (user == null) {
            throw new ServiiceException(Constants.CODE_401, "用户不存在，请重新登录");
        }
        // 用户密码加签验证 token
