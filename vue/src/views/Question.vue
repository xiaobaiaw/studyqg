<template>
  <div>
    <div style="margin: 10px 0">
      ...
      <el-select clearable v-model="courseId" placeholder="请选择" style="width: 200px" class="ml-5">
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"  @selection-change="handleSelectionChange">
      ...
      <el-table-column label="所属课程">
        <template v-slot="scorp">
          <span>{{ courses.find(v => v.id === scorp.row.courseId).name}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="试卷信息" :visible.sync="dialogFormVisible" width="40%" :close-on-click-modal="false">
      <el-form label-width="120px" size="small">
        <el-form-item label="类型">
          <el-select clearable v-model="form.type" placeholder="请选择题目类型" style="width: 100%">
            <el-option v-for="item in [{name: '选择题', value: 1}, {name: '判断题', value: 2}, {name: '问答题', value: 3},]"
                       :key="item.value" :label="item.name" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
